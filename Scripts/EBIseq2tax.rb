...
opts = OptionParser.new do |opt|
   opt.banner = "
Maps a list of EBI-supported IDs to their corresponding NCBI taxonomy using EBI
RESTful API. Avoid using this script on millions of entries at a time, since
each entry elicits a request to EBI's servers and a request to NCBI's servers.
...
def ebiFetch(db, id, format, outfile=nil)
   ...
   response = RestClient::Request.execute(:method=>:get,
      :url=>url, :timeout=>600)
   raise "Unable to reach EBI REST client, error code #{response.code}." unless
      response.code == 200
   response.to_s
end
def seq2taxid(id, db)
