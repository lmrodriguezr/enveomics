...
opts = OptionParser.new do |opt|
   ...
   when 'compile'
      ...
      opt.on("-d", "--data FILE", "Output file containing the bit-score thresholds. Required."){ |v| $o[:data]=v }
      ...
      opt.separator "   o The data file (-d) is a tab-delimited file containing five columns:"
   ...
   when 'filter'
      ...
      opt.on("-a", "--alignment FILE", "Protein alignment of the reference sequences. Required."){ |v| $o[:aln]=v }
      opt.on("-d", "--data FILE", "File containing the bit-score thresholds, produced with the compile task option -d. Required."){ |v| $o[:data]=v }
   ...
   when 'plot'
      ...
      opt.on("-a", "--alignment FILE", "Protein alignment of the reference sequences. Required."){ |v| $o[:aln]=v }
      opt.on("-t", "--table FILE", "Formated tabular file created by the compile task option -t. Required."){ |v| $o[:table]=v }
      opt.on("-d", "--data FILE", "File containing the bit-score thresholds, produced with the compile task option -d. Required."){ |v| $o[:data]=v }
      opt.on("-o", "--plot-file FILE", "File to be created with the plot. By default: value of -d + '.' + value of -f."){ |v| $o[:gout]=v }
      opt.on(      "--min-score NUMBER", "Minimum Bit-Score to consider a hit. By default: #{$o[:minscore]}"){ |v| $o[:minscore]=v.to_f }
...
class ROCWindow
   ...
   def initialize(data, from=nil, to=nil)
	 ...
	 b = to.nil? ? data.alnlen : [to,data.alnlen].min
	 @from = [a,b].min
	 @to = [a,b].max
	 self.compute!
      end
   ...
   def compute!
	 ...
	 self.rrun "rocobj <- roc(y$V5, y$V4, percent=TRUE, partial.auc=c(100, 90));"
   ...
   def to_s
      [self.from, self.to, self.hits, self.tps, self.thr].join("\t") + "\n"
   end
end

class ROCData
   attr_reader :alnlen, :windows, :r
   # Use ROCData.new(table,alnlen,window) to re-compute from table, use ROCData.new(data) to load
   def initialize(val, alnlen=nil, window=nil)
      if not alnlen.nil?
	 @alnlen = alnlen
	 @r = RInterface.new
   ...
   def init_windows!(size)
      @windows = []
      1.step(self.alnlen,size).each { |a| @windows << ROCWindow.new(self, a, a+size-1) }
   end
   def rrun(cmd, type=nil)
      self.r.run cmd, type
   end
...
class RInterface
   ...
   def run(cmd, type=nil)
	 ...
	 o += l
      ...
      case type
      when :float
	 /^\s*\[1\]\s+([0-9\.Ee+-]+).*/.match(o).nil? and abort "R error: expecting float, got #{o}"
	 return $1.to_f
      when :int
...
begin
   ...
   when 'compile'
	 ...
	 File.unlink $o[:table] if File.exists? $o[:table]
      end
      abort "-b/--blast is mandatory unless -t exists." if $o[:blast].nil? and not File.exists? $o[:table]
      abort "-d/--data is mandatory." if $o[:data].nil?
      bash "echo \"library('pROC')\" | #{$o[:r]} --vanilla", "Please install the 'pROC' library for R first."

      puts "Reading reference alignment." unless $o[:q]
      aln = Alignment.new
      aln.read_fasta $o[:aln]

      unless File.exists? $o[:table]
	 puts "Reading BLAST file and generating table." unless $o[:q]
	 blast2table($o[:blast], $o[:table], aln, $o[:minscore])
      end

      puts "Building windows." unless $o[:q]
      data = ROCData.new($o[:table], aln.cols, $o[:win])
      data.save $o[:data]
   when 'filter'
      abort "-a/--alignment is mandatory." if $o[:aln].nil?
      abort "-a/--alignment must exist." unless File.exists? $o[:aln]
      ...
      puts "Reading thresholds." unless $o[:q]
      data = ROCData.new $o[:data]
   ...
   when 'plot'
      ...
      abort "-a/--alignment must exist." unless File.exists? $o[:aln]
      abort "-d/--data is mandatory." if $o[:data].nil?
      abort "-t/--table is mandatory." if $o[:table].nil?
      ...
      $o[:gout] = "#{$o[:data]}.#{$o[:gformat]}" if $o[:gout].nil?
      # Open file
      r.run "#{$o[:gformat]}('#{$o[:gout]}', #{$o[:width]}, #{$o[:height]}#{extra});"
      r.run "layout(c(2,1,3), heights=c(2-1/#{aln.size},3,1));"
      # Read table
	 ...
	 r.run "x <- x[x$V4 >= #{$o[:minscore].to_s},];"
      ...
      r.run "plot(1, t='n', xlim=c(0.5,#{aln.cols}+0.5), ylim=range(x$V4)+c(-0.04,0.04)*diff(range(x$V4)), xlab='', ylab='Bit score', xaxs='i', xaxt='n');"
      ...
      r.run "w <- read.table('#{$o[:data]}', sep='\\t', h=F);"
	 ...
	 "lwd=c(1,NA,1,1),pch=c(NA,19,19,19),col=c('black','black','darkblue','darkred'),ncol=4,bty='n')"
      ...
      r.run "plot(1, t='n', xlim=c(0,#{aln.cols}),ylim=c(1,#{aln.seqs.size}),xlab='',ylab='Alignment',xaxs='i',xaxt='n',yaxs='i',yaxt='n',bty='n');"
      i = 0
      aln.seqs.values.each do |s|
         color = s.aln.split(//).map{|c| c=="-" ? "'grey80'" : ($o[:sbj].include?(s.id) ? "'red'" : "'black'")}.join(',')
	 r.run "rect((1:#{aln.cols-1})-0.5, rep(#{i}, #{aln.cols-1}), (1:#{aln.cols-1})+0.5, rep(#{i+1}, #{aln.cols-1}), col=c(#{color}), border=NA);"
	 i += 1
      end
	 ...
	 puts "  * accuracy: #{r.run "100*(sum(w$tp)+sum(w$tn))/(sum(w$p)+sum(w$n))", :float}%"
      ...
      r.run "plot(1, t='n', xlim=c(0,#{aln.cols}),ylim=c(50,100),xlab='Alignment position (amino acids)',ylab='Precision',xaxs='i');"
      if some_thr
