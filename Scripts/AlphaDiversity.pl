...
Usage:
   ...
   -i <str>	* Input table (columns:samples, rows:OTUs, first column:OTU names).
   -r <int>	Number of rows to ignore.  By default: 0.
   -c <int>	Number of columns to ignore after the first column (i.e., between
   		the first column, containing the name of the categories, and the
		first column of abundance values).  By default: 0.
   ...
   -D <str>	Distances file.  A squared matrix (or bottom-left half matrix) with
   		the distances between categories (OTUs or functions).  The first
		column must contain the names of the categories, and it shouldn't have
		headers.  If not set, all distances are assumed to be one.  Only used
		for Rao.
   -R		Do not calculate Rao indices.  This significantly decreases the total
   		running time. Note that Rao indices are highly susceptible to precision
		errors, and shouldn't be trusted for very big numbers.
   -q <int>	Estimate the qD index (true diversity of order q).  By default: 0.
...
for my $i (0 .. $#values){
   ...
   for my $ik (0 .. $#{$values[$i]}){
	 ...
	 for my $jk (0 .. $#{$values[$i]}){
	    my $dij = (!$o{D}) ? 1 : 
		      exists $D->{ $cats[$ik] }->{ $cats[$jk] } ? $D->{ $cats[$ik] }->{ $cats[$jk] } :
		      exists $D->{ $cats[$jk] }->{ $cats[$ik] } ? $D->{ $cats[$jk] }->{ $cats[$ik] } :
		      die "Cannot find distance between ".$cats[$ik]." and ".$cats[$jk].".\n";
