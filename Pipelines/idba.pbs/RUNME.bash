...
if [[ "$1" == "" || "$1" == "-h" || "$2" == "" ]] ; then
   ...
   folder	Path to the folder containing the 04.trimmed_fasta folder. The trimmed reads must
		be in interposed FastA format, and filenames must follow the format:
		<name>.CoupledReads.fa, where <name> is the name of the sample.
   data_type	Type of datasets in the project. One of: mg (for metagenomes), scg (for single-cell
...
for i in $dir/04.trimmed_fasta/*.CoupledReads.fa ; do
   ...
   jids[$k]=$(msub -v "SAMPLE=$b,FOLDER=$dir,TYPE=$TYPE,FA=$dir/04.trimmed_fasta/$b.CoupledReads.fa" -N "IDBA-$b" -l "mem=${RAM_G}g" $QUEUE $EXTRA $pac/run.pbs | grep .) ;
